...
public class StateCheckImp  {
    ...
    public static final String AVG_30_MIN_CALCULATED = "Avg30min";
    public static final String AVG_1_HR_CALCULATED = "Avg1hr";
    public static final String AVG_3_HR_CALCULATED = "Avg3hr";
    public static final String AVG_6_HR_CALCULATED = "Avg6hr";
    public static final String AVG_12_HR_CALCULATED = "Avg12hr";
    public static final String AVG_24_HR_CALCULATED = "Avg24hr";
...
//                    listener.onMotionEnds();
    ...
    public void calculateNewBreathAvgForState(long timestamp, Dao<TblBreathCounter, Integer> userDao){
        ...
        if(userDao!=null) {
            ...
            try {
                sizeOfBreathCount = userDao.queryForEq(TblState.FIELD_DATE, Calendar.getInstance().getTime()).size();
    ...
    private Observable<TblAverage> checkMovement(int sizeOfBreathCount, final Dao<TblBreathCounter, Integer> userDao, long timestamp) {
        return Observable.create(e -> {
            if (sizeOfBreathCount > 0){
                ...
                if(!Prefs.contains(AVG_30_MIN_CALCULATED)){
                    if(sizeOfBreathCount>AVG_30_MIN) {
                        Log.d(TAG,"Size AVG_30_MIN");
                        shouldCalculateAvg=true;
                        Prefs.putBoolean(AVG_30_MIN_CALCULATED, true);
                    }
                }else if(!Prefs.contains(AVG_1_HR_CALCULATED)){
                    if(sizeOfBreathCount>AVG_1_HR) {
                        Log.d(TAG,"Size AVG_1_HR");
                        shouldCalculateAvg=true;
                        Prefs.putBoolean(AVG_1_HR_CALCULATED, true);
                    }
                }else if(!Prefs.contains(AVG_3_HR_CALCULATED)){
                    if(sizeOfBreathCount>AVG_3_HR) {
                        Log.d(TAG,"Size AVG_3_HR");
                        shouldCalculateAvg=true;
                        Prefs.putBoolean(AVG_3_HR_CALCULATED, true);
                    }
                }else if(!Prefs.contains(AVG_6_HR_CALCULATED)){
                    if(sizeOfBreathCount>AVG_6_HR) {
                        Log.d(TAG,"Size AVG_6_HR");
                        shouldCalculateAvg=true;
                        Prefs.putBoolean(AVG_6_HR_CALCULATED, true);
                    }
                }else if(!Prefs.contains(AVG_12_HR_CALCULATED)){
                    if(sizeOfBreathCount>AVG_12_HR) {
                        Log.d(TAG,"Size AVG_12_HR");
                        shouldCalculateAvg=true;
                        Prefs.putBoolean(AVG_12_HR_CALCULATED, true);
                    }
                }else if(!Prefs.contains(AVG_24_HR_CALCULATED)){
                    if(sizeOfBreathCount>AVG_24_HR) {
                        Log.d(TAG,"Size AVG_24_HR");
                        shouldCalculateAvg=true;
                        Prefs.putBoolean(AVG_24_HR_CALCULATED, true);
                    }
                }
